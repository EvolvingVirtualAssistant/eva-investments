...
test('Should get class decorator with Command', () => {
...
test('Should register cli adapter without Command, as default adapter when calling class decorator', () => {
  const cliAdapter1 = cliAdapter();
...
test('Should register cli adapter with Command, when calling class decorator', () => {
  const cliAdapter1 = cliAdapter(MOCK_CLI_ADAPTER_COMMAND);
  const cliEntrypoint1 = cliEntrypoint(MOCK_CLI_ENTRYPOINT_COMMAND, false);
...
test('Should return singleton of class with cli adapter, regardless of multiple instantiations', () => {
  const cliAdapter1 = cliAdapter(MOCK_CLI_ADAPTER_COMMAND);
  const cliEntrypoint1 = cliEntrypoint(MOCK_CLI_ENTRYPOINT_COMMAND, false);
